function [BSmag,X,Y,Z,BX,BY,BZ] = BSmag_get_B(BSmag,X,Y,Z,h)
%---------------------------------------------------
%  NAME:      BSmag_get_B.m
...
for nF = 1:NF % Loop on each filament
    ...
    [M,N,P] = size(X);
%     h = waitbar(0,'Computing the magnetic field...');
%     h = waitbar(0,'Computing the magnetic field...');
    ...
    for m = 1:M
        for n = 1:N
            ...
            porcFilament = ((m-1)*N+(n-1)) /(M*N);
            waitbar( (nF-1)/NF + porcFilament/NF );
            ...
            DBx = zeros(1,length(x_P)-1);
            DBy = zeros(1,length(x_P)-1);
            DBz = zeros(1,length(x_P)-1);
